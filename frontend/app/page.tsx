    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {STEPS.map((label, idx) => (
          <div key={idx} className="flex items-center">
            <div className="flex flex-col items-center">
              <div
                className={[
                  ...
                  idx < currentStep
                    ...
                    : idx === currentStep
                      ? "border-pwc-orange bg-white text-pwc-orange"
              ...
              <span
                className={`mt-1 text-xs ${idx === currentStep ? "text-pwc-orange font-semibold" : "text-pwc-grey-20"}`}
              >
                {label}
              </span>
            </div>
            {idx < STEPS.length - 1 && (
              <div
                className={`w-16 h-0.5 mb-4 mx-1 transition-all ${idx < currentStep ? "bg-pwc-orange" : "bg-pwc-grey-20"}`}
              />
            )}
          </div>
        ))}
      ...
      {status.state === "idle" && (
        <div className="mt-6 rounded-lg border border-pwc-grey-20 bg-white p-4">
          <p className="text-xs font-semibold text-pwc-grey-70 mb-2">
            사용 방법
          </p>
          <ol className="text-xs text-pwc-grey-70 space-y-1 list-decimal list-inside">
            <li>
              국문 DSD 파일 (.dsd)과 영문 재무제표 (.docx 또는 .pdf)를
              업로드합니다.
            </li>
            <li>
              <strong className="text-pwc-black">대사 시작</strong> 버튼을
              클릭하면 자동으로 처리됩니다.
            </li>
